...
### Reembolso & Viagens — `/expenses`
- `GET /expenses`, `POST /expenses` (campo `kind`: `reembolso` | `viagem`)
- `POST /expenses/:id/submit` (monta funil de alçada por valor)
- `POST /expenses/:id/approve` (aprova etapa se houver alçada) · `POST /expenses/:id/reject`
- `POST /expenses/:id/pay` (reembolso — somente Diretoria Financeira/Admin)
- `GET /expense-policy` · `PUT /expense-policy` (parâmetros: diárias, R$/km, prazo, tetos)

Alçadas de despesa por valor: ≤ R$ 2 mil → Diretoria; ≤ R$ 10 mil → Diretoria + Diretoria Financeira; > R$ 10 mil → Diretoria Financeira → Sócios.
